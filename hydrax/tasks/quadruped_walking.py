...
class QuadrupedWalking(Task):
    ...
    def __init__(self) -> None:
        ...
        self.target_linear_velocity = jnp.array([0.2, 0.0])  # m/s in the local frame
    ...
    def _get_torso_angular_velocity_yaw_base(self, state: mjx.Data) -> jax.Array:
        ...
        vel = mjx._src.math.rotate(vel, mjx._src.math.quat_inv(state.xquat[self.torso_id]))  # to the base frame
        return vel[2]  # yaw angular velocity in base frame
    ...
    def _raibert_heuristic(self, state: mjx.Data) -> jax.Array:
        ...
        self.torso_pos = self._get_torso_position(state)  # (1, 3)
        self.hip_pos_base = jax.vmap(mjx._src.math.rotate, in_axes=(0, None))(
            self._get_hip_positions(state) - self.torso_pos, quat
        )  # (4, 3)
        self.torso_vel_base = mjx._src.math.rotate(self._get_torso_linear_velocity(state), quat)  # (3,)
        ...
        velocity_offset = self.torso_vel_base[:2] * stepping_time / 2.0  # (2,)
        feedback = 0.1 * (self.torso_vel_base[:2] - self.target_linear_velocity) # (2,)
        touchdown_pos_base_xy = self.hip_pos_base[:, :2] + velocity_offset[None, :] + feedback[None, :] # (4, 2)
        
        # Convert touchdown positions to world frame
        quat_inv = mjx._src.math.quat_inv(quat)
        touchdown_pos_base_xyz = jnp.concatenate([
            touchdown_pos_base_xy, 
            jnp.zeros((4, 1))  # z=0 in base frame (ground level)
        ], axis=1)  # (4, 3)
        touchdown_pos_world = jax.vmap(mjx._src.math.rotate, in_axes=(0, None))(
            touchdown_pos_base_xyz, quat_inv
        ) + self.torso_pos  # (4, 3)
        
        # Get current foot positions
        current_foot_pos = self._get_foot_positions(state)  # (4, 3)
        
        # Compute gait phase for each foot
        gait_phases = self._get_gait_phase(state.time, phases, cadence)  # (4,)
        
        # Interpolate between current position (stance) and desired touchdown position (swing)
        interpolated_positions = self._interpolate_foot_trajectory(
            current_foot_pos, touchdown_pos_world, gait_phases, duty_ratio, amplitude
        )
        
        return interpolated_positions

    def _get_gait_phase(self, time: float, phases: jax.Array, cadence: float) -> jax.Array:
        """Get the current gait phase for each foot.
        ...
        Returns:
            Current phase for each foot [0, 1] (4,)
        """
        # Calculate current phase for each foot
        current_phases = (time * cadence + phases) % 1.0
        return current_phases

    def _interpolate_foot_trajectory(
        self, 
        ...
        """Interpolate foot trajectory between stance and swing phases.
        
        Args:
            current_pos: Current foot positions (4, 3)
            touchdown_pos: Desired touchdown positions (4, 3)
            phases: Current gait phases for each foot (4,)
            duty_ratio: Fraction of time foot is on ground
            step_height: Maximum height during swing phase
            
        Returns:
            Interpolated foot positions (4, 3)
        """
        def single_foot_trajectory(current, touchdown, phase, duty_ratio, height):
            """Compute trajectory for a single foot."""
            # Determine if foot is in stance or swing phase
            is_swing = phase > duty_ratio
            
            # For swing phase, interpolate between current and touchdown position
            swing_progress = jnp.where(
                is_swing,
                (phase - duty_ratio) / (1.0 - duty_ratio),  # 0 to 1 during swing
                0.0
            )
            
            # Linear interpolation for x, y coordinates
            xy_interp = current[:2] + swing_progress * (touchdown[:2] - current[:2])
            
            # Parabolic trajectory for z coordinate during swing
            z_swing_height = 4 * height * swing_progress * (1 - swing_progress)  # Parabolic
            z_interp = jnp.where(
                is_swing,
                current[2] + z_swing_height,  # Add swing height during swing
                touchdown[2]  # Stay at ground level during stance
            )
            
            # During stance phase, stay at touchdown position
            final_pos = jnp.where(
                is_swing,
                jnp.array([xy_interp[0], xy_interp[1], z_interp]),
                touchdown  # Stay at touchdown position during stance
            )
        ...
        )(current_pos, touchdown_pos, phases, duty_ratio, step_height)
    ...
    def running_cost(self, state: mjx.Data, control: jax.Array) -> jax.Array:
        ...
        linear_velocity_cost = jnp.sum(jnp.square(self._get_torso_linear_velocity_xy_base(state) - self.target_linear_velocity))  # target forward velocity 0.5 m/s
        angular_velocity_cost = jnp.sum(jnp.square(self._get_torso_angular_velocity_yaw_base(state) - self.target_angular_velocity))  # target yaw angular velocity 0 rad/s
    ...
    def log_data(self, state: mjx.Data, control: jax.Array) -> Dict[str, jax.Array]:
        ...
        data_dict["angular_velocity_cost"] = jnp.sum(jnp.square(self._get_torso_angular_velocity_yaw_base(state) - self.target_angular_velocity))
        # data_dict["gait_cost"] = jnp.sum(((self._raibert_heuristic(state) - self._get_foot_positions(state)) / 0.05) ** 2)
        ...
        # print(f"step des", step_des)
        # print(f"foot pos", self._get_foot_positions(state))
